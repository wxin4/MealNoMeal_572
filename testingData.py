...
class features:
    ...
    def entropy(self, base=None):
        ...
        np.array(array).reshape((510, 1)) # reshape to the matrix that can be concatenated
...
def main():
    ...
    nomeal_df[31] = [0 for _ in range(len(nomeal))]
    meal_df[31] = [1 for _ in range(len(meal))]
    ...
    # Load Model and run the prediction from the training set
    result_list = joblib.load('ModelForTesting.pkl').predict(principalDf[[i for i in temp]])
    ...
    for pred in result_list:
        if pred == 1:
            ...
            mealnomeal = 'no meal'
            count_nomeal += 1
        print('Row# {0} is predicted as {1} which means the person might take {2}.'.format(row_num, int(pred), mealnomeal))
        row_num += 1
    print('Meal Predicted: {0}, No Meal Predicted: {1}'.format(count_meal, count_nomeal))
